...
class COMMON_MODEL extends CI_Model {
   ...
   public function get_data($tbl,$limit=0,$limit_start=0){
	 ...
	 public function get_datas($tbl,$field,$value){
		if(!empty($searh_query))
		{
			$this->db->like('user_type', $searh_query); 
		}
		if(!empty($field)){
			$this->db->where($field,$value);
		}
  ...
  public function change_status($table, $column, $value, $uniqueNameCol, $uniqueColValue)
	...
	public function send_otp_email($email,$token,$name)
	{
	    $this->load->library('email');
		$config['mailtype'] = 'html';
		$this->email->initialize($config);
		$this->email->from('satinderdeep.omninos@gmail.com');
		$this->email->to($email);
		$this->email->subject("Benevolentu - Regarding email verification.");
		$message = "Hi ".$name." your verification token here ".$token;		
		// echo"<pre>";
		// print_r($message);die;
